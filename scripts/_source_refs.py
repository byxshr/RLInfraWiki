...
REQUIRED_MANIFEST_FIELDS = ["id", "repo", "upstream_commit", "source_refs", "captured_at"]
...
def clone_name(manifest: dict[str, Any]) -> str | None:
    value = manifest.get("local_path") or manifest.get("local_path_hint")
    if not value:
        return None
    return Path(str(value)).parts[0]


def resolve_clone_root(manifest: dict[str, Any], source_root: Path) -> Path | None:
    name = clone_name(manifest)
    if not name:
        return None
    return (source_root / name).expanduser().resolve()
...
def validate_source_refs(
    *,
    ...
    for manifest in ref_manifests:
        ...
        claim_ids = {str(claim_id) for claim_id in manifest.get("claim_ids", []) or []}
        ...
        for idx, ref in enumerate(refs, 1):
            ...
            elif sha and not is_hex_sha(sha):
                issues.append(issue("error", "metadata", ref_label, f"sha256 must be 64 hex chars or {LEGACY_HASH_PLACEHOLDER!r}"))

            if not check_local or not clone_root or not clone_root.exists() or not line_range:
                continue
            ...
            if end > len(lines):
                ...
                continue
            if is_hex_sha(sha):
                actual = hash_source_lines(source_path, start, end)
                if actual != sha:
