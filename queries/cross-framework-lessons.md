...
| `framework-areal` | `areal` | AReaL framework profile maps source-reported async agentic framework capabilities to generic RLInfraWiki contracts and cross-framework lessons. |
| `framework-roll` | `roll` | ROLL framework profile maps source-reported Ray multi-role and async agentic capabilities to generic RLInfraWiki contracts and cross-framework lessons. |
...
| `pattern-async-rollout` | `areal`, `roll`, `slime`, `verl` | Pattern for asynchronous rollout where generation, tools, rewards, and training update do not advance in lockstep. |
