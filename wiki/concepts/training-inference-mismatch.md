---
id: concept-training-inference-mismatch
title: Training inference mismatch
type: concept
page_type: concept
components:
- training
- inference
summary: Divergence between rollout backend behavior and trainer-side probability
  or schema assumptions.
sources:
- repo-thudm-slime-readme
- doc-sglang-rl-systems
- repo-verl-readme
- repo-alibaba-roll-readme
- repo-inclusionai-areal-readme
created_at: '2026-06-13'
updated_at: '2026-06-13'
confidence: inferred
reproducibility: contract
version_sensitive: []
tags: []
risks: []
frameworks: []
backends: []
algorithms: []
deployment_modes: []
---

# Training inference mismatch

## Definition
Divergence between rollout backend behavior and trainer-side probability or schema assumptions.

## Infra contract
Track the term explicitly in task contracts, samples, logs, and validation evidence when it affects rollout/training correctness.

## Related pages
Use related capability, interface, failure-mode, and validation-pattern pages before making design decisions.
